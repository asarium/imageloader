...
TEST_F(STBITests, read_header_tga)
{
    ImgloadImage img;
    auto io = util::get_std_io();

    auto file_ptr = std::fopen(TEST_DATA_PATH "stb_image/FLAG_B24.TGA", "rb");
...
TEST_F(STBITests, read_data_tga)
{
    ImgloadImage img;
    auto io = util::get_std_io();
...
TEST_F(STBITests, read_data_tga_flip)
{
    this->makeContext(IMGLOAD_CONTEXT_FLIP_IMAGES);
