...
class PCXTests : public util::ContextFixture
{
};

TEST_F(PCXTests, read_header)
{
    ImgloadImage img;
    auto io = util::get_std_io();

    auto file_ptr = std::fopen(TEST_DATA_PATH "pcx/test.pcx", "rb");
    ...
    // Change the assertions below to match the test file you are using

    uint32_t val;
    ASSERT_EQ(IMGLOAD_ERR_NO_ERROR,
              imgload_image_get_property(img, 0, IMGLOAD_PROPERTY_WIDTH, IMGLOAD_PROPERTY_TYPE_UINT32, &val));
    ASSERT_EQ(800, val);

    ASSERT_EQ(IMGLOAD_ERR_NO_ERROR,
              imgload_image_get_property(img, 0, IMGLOAD_PROPERTY_HEIGHT, IMGLOAD_PROPERTY_TYPE_UINT32, &val));
    ASSERT_EQ(600, val);

    ASSERT_EQ(IMGLOAD_ERR_NO_ERROR,
              imgload_image_get_property(img, 0, IMGLOAD_PROPERTY_DEPTH, IMGLOAD_PROPERTY_TYPE_UINT32, &val));
    ASSERT_EQ(1, val);
...
TEST_F(PCXTests, read_data_flip)
{
